...
Output files:
  ...
  06_plddt_ipsae.png        — 6-panel: AF2 initial guess pLDDT,
...
# ─────────────────────────────────────────────────────────────────────────────
# Plot 06: pLDDT + ipSAE  (6 panels)
#
# Panel layout (1 row × 6 cols):
...
def plot_plddt_ipsae(df_stats: pd.DataFrame,
    ...
    df = df_stats[['design', 'ipsae_binder_peptide', 'ipsae_n_contacts',
                  'af3_iptm', 'af3_ranking_score']].merge(
    ...
    fig, axes = plt.subplots(2, 3, figsize=(8, max(5, n * 0.3)), sharey=True)

    # ── Panel 0: AF2 initial guess pLDDT (binder) ───────────────────────────
    ax = axes[0, 0]
    ...
    # ── Panel 1: AF2 initial guess pLDDT (target) ───────────────────────────
    ax = axes[0, 1]
    ...
    # ── Panel 2: Binder aligned RMSD (lower = better) ───────────────────────
    ax = axes[0, 2]
    ...
    # ── Panel 3: Target aligned RMSD (lower = better) ───────────────────────
    ax = axes[1, 0]
...
def main():
    ...
    plot_plddt_ipsae(df_stats, df_spec,
