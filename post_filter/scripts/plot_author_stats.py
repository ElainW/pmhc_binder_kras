"""
plot_author_stats.py
====================
Produces separate PNG figures for author design stats.
...
For hbonds_int, delta_unsatHbonds (plot 03) and buns_delta_unsat (plot 04):
...
def plot_charge_cysteine(df_epi: pd.DataFrame, df_xlsx: pd.DataFrame,
                         out_path: str):
    xlsx_seq = df_xlsx.set_index('Binder name')['Amino acid sequence'].to_dict()
    rows = []
...
# ─────────────────────────────────────────────────────────────────────────────
# Shared bar plot helper with optional per-residue annotation
# ─────────────────────────────────────────────────────────────────────────────
...
def _barh_panel(ax, labels, vals, norm_vals_for_color, title,
    ...
    """
    Draw a horizontal bar chart panel.

    vals                  — bar lengths (raw values)
    norm_vals_for_color   — values used for green/red coloring (can differ from vals)
    bar_annotations       — if provided, these numbers are shown on bars instead of vals
                            (used for per-residue normalized values)
    annotation_suffix     — appended to each annotation string
    """
    vrange = vals.max() - vals.min()
    if lower_better:
        colors = plt.cm.RdYlGn_r(norm_vals_for_color)
    else:
        colors = plt.cm.RdYlGn(norm_vals_for_color)
...
# Metrics that use raw bar values but show per-residue annotation on the bar
# key: column name, value: normalisation column
FASTRELAX_PER_RES = {
    'hbonds_int':        'nres_int',
...
def plot_fastrelax(df_stats: pd.DataFrame, out_path: str):
    metrics = [
        ...
        ('hbonds_int',            'H-bonds at interface\n(bar=raw, label=per interface res)', False),
        ('delta_unsatHbonds',     'ΔUnsatisfied H-bonds\n(bar=raw, label=per interface res)', True),
    ...
    for ax, (col, title, lower_better) in zip(axes, metrics):
        # Column name resolution (handles slash variants)
        actual_col = col
        ...
        # Per-residue annotation for selected metrics
        if actual_col in FASTRELAX_PER_RES:
            norm_col = FASTRELAX_PER_RES[actual_col]
            if norm_col in df.columns:
                norm_denom = df[norm_col].replace(0, np.nan).values
                bar_annotations = vals / norm_denom
            else:
                bar_annotations = None
        else:
            bar_annotations = None
    ...
    if FLAGGED:
        fig.text(0.5, -0.01,
                 f'* flagged designs: {", ".join(sorted(FLAGGED))} '
                 f'(prame-9 and sars-6 include B2M in complex)',
                 ha='center', fontsize=8, color='grey')
...
# buns_delta_unsat: bar = raw, annotation = per nres_all
BINDCRAFT_PER_RES = {
...
def plot_bindcraft(df_stats: pd.DataFrame, out_path: str):
    ...
    for ax, (col, title, lower_better) in zip(axes, metrics):
        ...
        # Per-residue annotation for buns_delta_unsat
        ...
        # BindCraft threshold (use original key name for lookup)
        thresh = BINDCRAFT_THRESHOLDS.get(col)
        thresh_label = f'BindCraft thresh={thresh}' if thresh is not None else None
...
def plot_plddt_ipsae(df_plddt: pd.DataFrame, df_stats: pd.DataFrame,
    ...
    fig.text(0.5, -0.02,
             f'* flagged (anomalous ipSAE — back-face docking or B2M inclusion): '
...
def main():
    ...
    df_xlsx  = pd.read_csv(args.xlsx)
    df_plddt = pd.read_csv(args.plddt_tsv,  sep='\t')

    plot_charge_cysteine(df_epi, df_xlsx,
    ...
    plot_plddt_ipsae(df_plddt, df_stats,
